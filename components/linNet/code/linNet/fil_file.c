...
static boolean checkForInstallDir(const char *candidate)
{
#if 0
...
# define PRINTF(formatStr, ...)
    ...
    char appName[strlen(candidate) + strlen(LIN_APP_FILE_NAME) + sizeof(SL)];
    snprintf(appName, sizeof(appName), "%s" SL LIN_APP_FILE_NAME, candidate);
...
boolean fil_copyFile(const char * const targetFilePath, const char * const srcFile)
{
#if 0
...
boolean fil_copyDir(const char * const targetDir, const char * const srcDir)
{
#if 0
 ...
 *   If the application related environment variable LINNET_HOME is set it is expected to
...
# define PRINTF(...)
    ...
    /* The user should have set an environment variable that points to the installation
       directory. */
    const char * const home = getenv(LIN_ENV_VAR_HOME);
    if(home != NULL  && checkForInstallDir(home))
    {
        PRINTF( "Installation dir %s found via enviroment variable " LIN_ENV_VAR_HOME "\n"
              , home
              );
    ...
    {
        PRINTF("Environment variable " LIN_ENV_VAR_HOME " not set or wrong\n");
    }
